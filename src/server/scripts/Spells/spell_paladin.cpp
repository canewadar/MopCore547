        ...
        class spell_pal_ardent_defender_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffectPtr aurEff, DamageInfo & dmgInfo, uint32 & absorbAmount)
            {
                ...
                if (victim->GetTypeId() == TYPEID_UNIT/* && victim->ToCreature()->IsNPCBot()*/)
                {
                    ...
                    if (remainingHealth <= 0 && !victim->ToCreature()->HasSpellCooldown(PALADIN_SPELL_ARDENT_DEFENDER_HEAL))//GetSpellHistory()->HasCooldown(PAL_SPELL_ARDENT_DEFENDER_HEAL))
                    {
                        // Cast healing spell, completely avoid damage
                        absorbAmount = dmgInfo.GetDamage();

						//uint32 defenseSkillValue = victim->GetUnitMeleeSkill(); //->GetDefenseSkillValue();
                        ...
                        victim->ToCreature()->AddBotSpellCooldown(PALADIN_SPELL_ARDENT_DEFENDER_HEAL, 120 * IN_MILLISECONDS);
                    }
