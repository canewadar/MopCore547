...
public:
    ...
    struct priest_botAI : public bot_minion_ai
    {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            if (spellId == WEAKENED_SOUL_DEBUFF)
                if (me->getLevel() >= 51)
                    if (Aura* soul = target->GetAura(WEAKENED_SOUL_DEBUFF).get())
            ...
            if (spellId == GetSpell(MIND_FLAY_1))
                if (me->getLevel() >= 51)
                    if (Aura* pain = target->GetAura(GetSpell(SW_PAIN_1), me->GetGUID()).get())
                        pain->RefreshDuration();

            if (spellId == GetSpell(FEAR_WARD_1))
            {
                //2 minutes bonus duration for Fear Ward
                if (Aura* ward = target->GetAura(GetSpell(FEAR_WARD_1), me->GetGUID()).get())
                {
                ...
                spellId == GetSpell(SHADOW_PROTECTION_1) || spellId == GetSpell(DIVINE_SPIRIT_1))
            {
                ...
                if (Aura* buff = target->GetAura(spellId, me->GetGUID()).get())
                {
  ...
  /*Talent*/lvl >= 60 ? InitSpellMap(GUARDIAN_SPIRIT_1) : RemoveSpell(GUARDIAN_SPIRIT_1);
            ...
            //InitSpellMap(NORMAL_HEAL_1); // No SpellInfo
            ...
            //InitSpellMap(PW_FORTITUDE_1); // No SpellInfo
