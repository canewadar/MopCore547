...
public:
	...
	std::vector<ChatCommand> GetCommands() const
	{
		...
		{
			{ "faction",    SEC_VIP, false, &HandleNpcSetFactionCommand, "" },
			{ "owner",      SEC_VIP, false, &HandleNpcSetOwnerCommand,   "" },
		};
	...
	*/
	ChatCommand* GetCommands() const
	{
		...
		{
			{ "faction",    SEC_GAMEMASTER, false, &HandleNpcSetFactionCommand, "" },
			{ "owner",      SEC_GAMEMASTER, false, &HandleNpcSetOwnerCommand,   "" },
	...
	static bool HandleNpcSetOwnerCommand(ChatHandler* handler, const char* args)
	{
		...
		if (guidlow)
			sObjectMgr->GetPlayerNameByGUID(ObjectGuid(HighGuid::HIGHGUID_PLAYER, guidlow), characterName);
		else
		...
		handler->PSendSysMessage("%s's new owner is %s (guidlow: %u)", bot->GetName(), characterName.c_str(), guidlow);
		return true;
	}
	...
	static bool HandleNpcBotDeleteCommand(ChatHandler* handler, const char* /*args*/)
	{
		...
		for (uint8 i = 0; i != BOT_INVENTORY_SIZE; ++i)
		{
			...
			{
				handler->PSendSysMessage("%s still has eqipment assigned. Please remove equips before deleting bot!", bot->GetName());
				handler->SetSentErrorMessage(true);
				return false;
			}
		}
	...
	static bool HandleNpcBotSpawnCommand(ChatHandler* handler, const char* args)
	{
		...
		if (!creature->Create(/*map*/sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT)/*()*/, map, chr->GetPhaseMgr().GetPhaseMaskForSpawn(), id, 0, 0, x, y, z, o))
		{
			delete creature;
			return false;
		}
	...
	static bool HandleNpcBotJumpCommand(ChatHandler* handler, const char* /*args*/)
	{
		...
		bot->GetMotionMaster()->MoveJump(/**player*/player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), speedXY, speedZ);
	...
	static bool HandleNpcBotInfoCommand(ChatHandler* handler, const char* /*args*/)
	{
		...
		handler->PSendSysMessage("Listing NpcBots for %s", master->GetName());
