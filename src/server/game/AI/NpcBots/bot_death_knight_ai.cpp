    ...
    struct death_knight_botAI : public bot_minion_ai
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (runicpowertimer2 <= diff)
            {
                if (me->IsInCombat())
                {
                    if (getpower() < (uint32)me->GetMaxPower(POWER_RUNIC_POWER))
            ...
            {
                Aura* horn = master->GetAura(GetSpell(HORN_OF_WINTER_1)).get();
            ...
            {
                Aura* bone = me->GetAura(GetSpell(BONE_SHIELD_1)).get();
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            {
                if (Aura* horn = target->GetAura(spellId, me->GetGUID()).get())
                {
            ...
            {
                if (Aura* fever = target->GetAura(spellId, me->GetGUID()).get())
                {
            ...
            if (spellId == ICY_CLUTCH)
            {
                if (target->GetTypeId() != TYPEID_PLAYER)
                {
                    if (Aura* chill = target->GetAura(spellId, me->GetGUID()).get())
                    {
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell)
        {
            ...
            if (spellId == GetSpell(ANTI_MAGIC_SHELL_1))
            {
                ...
                if (Aura* shell = me->GetAura(spellId).get())
                {
            ...
            if (spellId == GetSpell(VAMPIRIC_BLOOD_1))
            {
                ...
                if (Aura* blood = me->GetAura(spellId).get())
                {
            ...
            if (spellId == GetSpell(BONE_SHIELD_1))
            {
                ...
                if (Aura* bone = me->GetAura(spellId).get())
                {
                ...
                spellId == ICY_TALONS_AURA3 || spellId == ICY_TALONS_AURA4 || spellId == ICY_TALONS_AURA5)
            {
                ...
                if (Aura* talons = me->GetAura(spellId).get())
                {
            ...
            if (spellId == GetSpell(ICEBOUND_FORTITUDE_1))
            {
                ...
                if (Aura* fort = me->GetAura(spellId).get())
                {
