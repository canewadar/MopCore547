        ...
        void ApplyClassDamageMultiplierSpell(int32& damage, SpellNonMeleeDamage& /*damageinfo*/, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool& crit) const
        {
            ...
            if (!crit)
            {
                ...
                if (lvl >= 10 &&
                    ...
                    spellId == GetSpell(CONEOFCOLD_1)/* || spellId == ARCANEBLAST || spellId == SCORCH*/))
                    aftercrit += 6.f;
                ...
                if (lvl >= 15 &&
                    ...
                    spellId == GetSpell(DRAGONBREATH_1)/* || spellId == ARCANEXPLOSION ||
                    ...
                    switch (spell->Effects[i].Effect)
                    {
                        ...
                        case SPELL_EFFECT_CREATE_ITEM_2:
                        {
                            ...
                            if (newitemid)
                            {
                                ...
                                if (msg != EQUIP_ERR_OK)
                                {
                                    if (msg == EQUIP_ERR_INV_FULL/*EQUIP_ERR_INVENTORY_FULL*/ || msg == EQUIP_ERR_ITEM_MAX_COUNT/*EQUIP_ERR_CANT_CARRY_MORE_OF_THIS*/)
                                        count -= no_space;
                                    else
                                    {
