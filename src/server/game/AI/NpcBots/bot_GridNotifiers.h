        ...
        bool operator()(Unit const* u) const
        {
                ...
                ai->IsInBotParty(u->GetVictim()))
            {
                ...
                for (Unit::AuraMap::const_iterator itr = Auras.begin(); itr != Auras.end(); ++itr)
                {
                    ...
                    Info = itr->second->GetSpellInfo();//aura->GetSpellInfo();
                    id = Info->Id;
                    ...
                    AuraApplication const* aurApp = itr->second->GetApplicationOfTarget(u->GetGUID());//aura->GetApplicationOfTarget(u->GetGUID());
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (u->IsAlive() && checker->IsWithinDistInMap(u, m_range))
            {
                ...
                {
                    //Aura* aura = itr->second;
                    if (itr->second->GetId() == spell)//aura->GetId() == spell)
                        if (caster == 0 || itr->second->GetCasterGUID() == caster)//aura->GetCasterGUID() == caster)
                            return true;
                }
            }
            return false;
