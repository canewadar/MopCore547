    ...
    struct rogue_botAI : public bot_minion_ai
    {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            if (spellId == GetSpell(SLICE_DICE_1))
            {
                ...
                if (Aura* dice = me->GetAura(GetSpell(SLICE_DICE_1)).get())
                {
            ...
            else if (spellId == GetSpell(RUPTURE_1))
            {
                ...
                if (Aura* rupture = target->GetAura(GetSpell(RUPTURE_1), me->GetGUID()).get())
                {
            ...
            {
                if (Aura* kidney = target->GetAura(GetSpell(KIDNEY_SHOT_1), me->GetGUID()).get())
                {
