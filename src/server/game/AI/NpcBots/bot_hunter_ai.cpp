...
public:
    ...
    struct hunter_botAI : public bot_minion_ai
    {
        ...
        void ApplyClassDamageMultiplierMelee(int32& damage, SpellNonMeleeDamage& damageinfo, SpellInfo const* spellInfo, WeaponAttackType attackType, bool& crit) const
        {
            ...
            if (lvl >= 40)
                (static_cast<ManaRestoreList>(TotH))[spellId] = crit && attackType == RANGED_ATTACK ?
					spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask(), me->GetSpellPowerEntryBySpell(spellInfo)) / 2 : 0;
        }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            if (spellId == GetSpell(CONCUSSIVE_SHOT_1))
            {
                ...
                if (Aura* concus = target->GetAura(spellId, me->GetGUID()).get())
                {
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell)
        {
            ...
            if (spellId == GetSpell(RAPID_FIRE_1))
            {
                ...
                if (Aura* rapid = me->GetAura(spellId).get())
                {
