...
public:
    ...
    struct warrior_botAI : public bot_minion_ai
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (ragetimer <= diff)
            {
                ...
                {
                    if (me->GetPower(POWER_RAGE) > int32(10.f * rageLossMult))
        ...
        int32 rcost(uint32 spellId) const
        {
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
                return spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask(), me->GetSpellPowerEntryBySpell(spellInfo));
            return 0;
        }
        ...
        void Attack(uint32 diff)
        {
            ...
            {
                Aura* sunder = opponent->GetAura(GetSpell(SUNDER_1), me->GetGUID()).get();
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell)
        {
            ...
            if (spellId == GetSpell(SHIELD_WALL_1))
            {
                ...
                if (Aura* wall = me->GetAura(spellId).get())
                {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            if (spellId == GetSpell(PIERCING_HOWL_1))
            {
                ...
                {
                    if (Aura* howl = target->GetAura(spellId, me->GetGUID()).get())
                    {
            ...
            if (spellId == GetSpell(BATTLESHOUT_1) || spellId == GetSpell(COMMANDING_SHOUT_1))
            {
                if (me->getLevel() >= 15)
                {
                    ...
                    AuraEffect* eff = target->GetAuraEffect(spellId, EFFECT_0, me->GetGUID()).get();
                    if (eff)
                    {
            ...
            if (spellId == GetSpell(THUNDER_CLAP_1))
            {
                ...
                {
                    AuraEffect* eff = target->GetAuraEffect(spellId, EFFECT_1, me->GetGUID()).get();
                    if (eff)
                    {
                        int32 amount = eff->GetAmount();
            ...
            if (spellId == GetSpell(BATTLESHOUT_1) || spellId == GetSpell(COMMANDING_SHOUT_1))
            {
                ...
                if (Aura* shout = target->GetAura(spellId, me->GetGUID()).get())
                {
            ...
            if (spellId == GetSpell(REND_1))
            {
                ...
                if (Aura* rend = target->GetAura(spellId, me->GetGUID()).get())
                {
            ...
            if (spellId == GetSpell(INTERVENE_1))
            {
                ...
                if (Aura* vene = target->GetAura(spellId, me->GetGUID()).get())
                {
        ...
        void InitSpells()
        {
            ...
            InitSpellMap(CHARGE_1); // No SpellInfo
            InitSpellMap(OVERPOWER_1);
   /*Quest*/lvl >= 14 ? InitSpellMap(TAUNT_1) : RemoveSpell(TAUNT_1);
            //InitSpellMap(BLOODRAGE_1); // No SpellInfo
            InitSpellMap(BERSERKERRAGE_1);
            //InitSpellMap(INTERCEPT_1); // No SpellInfo
  ...
  /*Talent*/lvl >= 60 ? InitSpellMap(BLADESTORM_1) : RemoveSpell(BLADESTORM_1);
            ...
            //InitSpellMap(REND_1); // No SpellInfo
            InitSpellMap(EXECUTE_1);
            InitSpellMap(PUMMEL_1);
  /*Talent*/lvl >= 5 ? InitSpellMap(MORTALSTRIKE_1) : RemoveSpell(MORTALSTRIKE_1);
            InitSpellMap(SLAM_1);
   /*Quest*/lvl >= 10 ? InitSpellMap(SUNDER_1) : RemoveSpell(SUNDER_1);
  /*Talent*/lvl >= 15 ? InitSpellMap(SWEEPING_STRIKES_1) : RemoveSpell(SWEEPING_STRIKES_1);
 ...
 /*Special*/lvl >= 20 ? InitSpellMap(RETALIATION_1) : RemoveSpell(RETALIATION_1);
  ...
  /*Talent*/lvl >= 20 ? InitSpellMap(LAST_STAND_1) : RemoveSpell(LAST_STAND_1);
            ...
            //InitSpellMap(SHIELD_BASH_1); // No SpellInfo
            InitSpellMap(HEROIC_THROW_1);
  /*Talent*///lvl >= 30 ? InitSpellMap(CONCUSSION_BLOW_1) : RemoveSpell(CONCUSSION_BLOW_1); // No SpellInfo
  /*Talent*///lvl >= 40 ? InitSpellMap(VIGILANCE_1) : RemoveSpell(VIGILANCE_1); // No SpellInfo
  /*Talent*/lvl >= 50 ? InitSpellMap(DEVASTATE_1) : RemoveSpell(DEVASTATE_1);
            //InitSpellMap(MOCKING_BLOW_1); // No SpellInfo
  ...
  /*Talent*/lvl >= 20 ? InitSpellMap(PIERCING_HOWL_1) : RemoveSpell(PIERCING_HOWL_1);
            ...
            //InitSpellMap(CHALLENGING_SHOUT_1); // No SpellInfo
