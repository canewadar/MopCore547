...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    MoPCore::AnyPlayerInObjectRangeCheck checker(m_capturePoint, radius);
    MoPCore::PlayerListSearcher<MoPCore::AnyPlayerInObjectRangeCheck> searcher(m_capturePoint, players, checker);
    ...
    for (uint8 team = 0; team != 2; ++team)
    {
        ...
        for (PlayerSet::const_iterator itr = m_activePlayers[team].begin(); itr != m_activePlayers[team].end(); ++itr)
        {
            if (Player* player = *itr)//ObjectAccessor::FindPlayer(itr))
