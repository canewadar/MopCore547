...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = MoPCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
    CellCoord cellCoord = MoPCore::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = MoPCore::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<MoPCore::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<MoPCore::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    MoPCore::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<MoPCore::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<MoPCore::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
...
{
    cell.SetNoCreate();
    MoPCore::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<MoPCore::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    MoPCore::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<MoPCore::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<MoPCore::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridCoord p = MoPCore::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
        ...
        {
            GridCoord p2 = MoPCore::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
