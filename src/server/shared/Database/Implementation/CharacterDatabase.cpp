...
void CharacterDatabaseConnection::DoPrepareStatements()
{
	...
	// New NPCBots
	PREPARE_STATEMENT(CHAR_SEL_NPCBOTS,          "SELECT entry FROM characters_npcbot",                        CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_SEL_NPCBOT_OWNER,     "SELECT owner FROM characters_npcbot WHERE entry = ?",        CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_UPD_NPCBOT_OWNER,     "UPDATE characters_npcbot SET owner = ? WHERE entry = ?",     CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_UPD_NPCBOT_OWNER_ALL, "UPDATE characters_npcbot SET owner = ? WHERE owner = ?",     CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_NPCBOT_ROLES,     "SELECT roles FROM characters_npcbot WHERE entry = ?",        CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_UPD_NPCBOT_ROLES,     "UPDATE characters_npcbot SET roles = ? WHERE entry = ?",     CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_NPCBOT_EQUIP,     "SELECT equipMhEx, equipOhEx, equipRhEx, "
	...
	PREPARE_STATEMENT(CHAR_SEL_NPCBOT_EQUIP_BY_ITEM_INSTANCE, "SELECT ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomPropertyId, ii.durability, ii.playedTime, ii.text, ii.guid, ii.itemEntry, ii.owner_guid "
	...
	PREPARE_STATEMENT(CHAR_UPD_NPCBOT_EQUIP,     "UPDATE characters_npcbot SET equipMhEx = ?, equipOhEx = ?, equipRhEx = ?, "
	...
	PREPARE_STATEMENT(CHAR_DEL_NPCBOT,           "DELETE FROM characters_npcbot WHERE entry = ?",              CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_INS_NPCBOT,           "INSERT INTO characters_npcbot (entry, roles) VALUES (?, ?)", CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_UPD_NPCBOT_FACTION,   "UPDATE characters_npcbot SET faction = ? WHERE entry = ?",   CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_SEL_NPCBOT_FACTION,   "SELECT faction FROM characters_npcbot WHERE entry = ?",      CONNECTION_SYNCH);
	// END New NPCBots
}
